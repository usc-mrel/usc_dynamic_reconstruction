function sens_map = get_sens_map(im,options, smooth, niter)
...
switch options
    case '2D'
        ...
        sens_map(:,:,1,:) = ismrm_estimate_csm_walsh_optimized_yt(im_for_sens,smooth, niter);
    case '3D'
        [sx,sy,sz,~,coils] = size(im);
        im_for_sens = squeeze(sum(im,4));
        sens_map = single(zeros(sx,sy,sz,1,coils));
        for i=1:sz
            sens_map(:,:,i,1,:) = ismrm_estimate_csm_walsh_optimized_yt(squeeze(im_for_sens(:,:,i,:)),smooth, niter);
%             sens_map(:,:,i,1,:) = ismrm_estimate_csm_walsh(squeeze(im_for_sens(:,:,i,:)));
        end
    case 'SMS'
