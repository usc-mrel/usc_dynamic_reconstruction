...
%% scale trajectory by over sampling
% YT: 2021/05/26 plus 0.5 to correct for phase. Haven't figured out
% YT: 2022/08/02 plus 1 to correct for phase. Haven't figured out
% kx = kx * over_sampling;
% ky = ky * over_sampling;
kx = (kx + 1) * over_sampling;
ky = (ky + 1) * over_sampling;
...
phase = exp(-1i * pi * (x_cart + y_cart) * (sx - 1) / skx);
...
ind_cart = sub2ind([skx, skx, nof], x_cart, y_cart);
ind_cart = permute(ind_cart, [1, 2, 4, 3]);
ind_cart = bsxfun(@plus, ind_cart, permute(0:skx*skx:skx*skx*(nof-1),[3, 4, 1, 2]));

ind_ncart = repmat((1:Nsamples*Nrep).', [prod(kernel_size),1]);
ind_ncart = bsxfun(@plus,ind_ncart,0:Nsamples*Nrep:Nsamples*Nrep*(nof-1));

S = sparse(ind_cart(:),ind_ncart(:),weight_kb(:),skx*skx*nof,Nsamples*Nrep*nof);
...
%%%
% YT: 2021/05/26 take negtive to correct for phase, don't know why yet
% N.S = S;
% YT: 2023/07/06 change back to S
N.S = S;
...
% N.W = designFilter(skx,-1,'ram-lak');
