function N = init_new(kx,ky,over_sampling,core_size,kernel)
...
dx = round((dx+core_size(1)/2)*100)+1;
dy = round((dy+core_size(1)/2)*100)+1;
idx = sub2ind(core_size*100+1,dx,dy);
weight_kb = double(kernel(idx));
weight_kb = weight_kb.*phase;
weight_kb = permute(weight_kb,[1,2,4,3]);
weight_kb = reshape(weight_kb,sx*nor*prod(core_size),nof);
...
%N.Apodizer = N.Apodizer/max(N.Apodizer(:));
%%%
N.S = S;
%N.rad2cart = rad2cart;
